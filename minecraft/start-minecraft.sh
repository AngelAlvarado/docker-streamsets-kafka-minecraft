...
# If we have a bootstrap.txt file... feed that in to the server stdin
#exec java $JVM_OPTS -jar $SERVER --noconsole
if [ -f /data/bootstrap.txt ];
then
    exec java $JVM_OPTS -classpath /kafka/slf4j-api-1.7.22.jar:/kafka/kafka-clients-0.9.0.1.jar:$SERVER org.bukkit.craftbukkit.Main "$@" $EXTRA_ARGS  < /data/bootstrap.txt
else
    exec java $JVM_OPTS -classpath /kafka/slf4j-api-1.7.22.jar:/kafka/kafka-clients-0.9.0.1.jar:$SERVER org.bukkit.craftbukkit.Main "$@" $EXTRA_ARGS
fi
